...
>(({ className, children, ...props }, ref) => (
  <DrawerPortal>
    ...
    <DrawerPrimitive.Content
      ref={ref}
      className={cn(
        "fixed inset-x-0 bottom-0 z-50 flex h-auto max-h-[86vh] flex-col rounded-t-2xl border border-border bg-card shadow-raised",
        className
      )}
      {...props}
    >
...
const DrawerHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("grid gap-1 p-5 text-left", className)} {...props} />
);
